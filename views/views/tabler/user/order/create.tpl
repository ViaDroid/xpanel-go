                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <table class="table table-transparent table-responsive">
                                ...
                                <tr>
                                    <td>商品类型</td>
                                    <td class="text-end">{{.product.TypeText}}</td>
                                </tr>
                                {{if or (eq "tabp" .product.Type)  (eq "time" .product.Type)}}
                                    <tr>
                                        <td>商品时长</td>
                                        <td class="text-end">{{.product.ContentMap.time}} 天</td>
                                    </tr>
                                    <tr>
                                        <td>等级时长</td>
                                        <td class="text-end">{{.product.ContentMap.class_time}} 天</td>
                                    </tr>
                                    <tr>
                                        <td>等级</td>
                                        <td class="text-end">Lv. {{.product.ContentMap.class}}</td>
                                    </tr>
                                {{end}}
                                {{if or (eq "tabp" .product.Type)  (eq "bandwidth" .product.Type)}}
                                    <tr>
                                        <td>可用流量</td>
                                        <td class="text-end">{{.product.ContentMap.bandwidth}} GB</td>
                                    </tr>
                                {{end}}
                                {{if or (eq "tabp" .product.Type)  (eq "time" .product.Type)}}
                                    <tr>
                                        <td>速率限制</td>
                                        {{if czeq 0 .product.ContentMap.speed_limit}}
                                            <td class="text-end">不限制</td>
                                        {{else}}
                                            <td class="text-end">{{.product.ContentMap.speed_limit}} Mbps</td>
                                        {{end}}
                                    </tr>
                                    <tr>
                                        <td>同时连接 IP 限制</td>
                                        {{if czeq 0 .product.ContentMap.ip_limit}}
                                            <td class="text-end">不限制</td>
                                        {{else}}
                                            <td class="text-end">{{.product.ContentMap.ip_limit}}</td>
                                        {{end}}
                                    </tr>
                                {{end}}
                            </table>
