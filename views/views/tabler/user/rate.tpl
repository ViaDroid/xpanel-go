...
<div class="page-wrapper">
                                ...
                                <div class="ms-auto">
                                    <div class="dropdown">
                                        <a id="dropdown-toggle" class="dropdown-toggle text-secondary" href="#" data-bs-toggle="dropdown"
                                           aria-haspopup="true" aria-expanded="false">{{index (index .node_list 0) "name"}}</a>
                                        <div class="dropdown-menu dropdown-menu-end">
                                            {{range  $node := .node_list}}
                                            <a class="dropdown-item" hx-post="/user/rate" hx-swap="none"
                                                hx-vals='{ "node_id": "{{index $node "id"}}" }'>
                                                {{index $node "name"}}
                                            </a>
                                            {{end}}
                                        </div>
    ...
    <script src="//{{index .config "jsdelivr_url"}}/npm/@tabler/core@latest/dist/libs/apexcharts/dist/apexcharts.min.js"></script>
